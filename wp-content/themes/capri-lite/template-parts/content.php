    ...
    <div class="entry-content<?php echo ! is_single() ? ' entry-content-blog' : ''; ?>">
        ...
        if ( $capri_more || is_single() ) {
            ...
            if ( is_array( $acf_fields ) ) {
                $flag_empty_option = true;
                echo '<p align="center">';
                foreach ( $acf_fields as $field ) {
                    if ( ! empty( $field['label'] ) and ! empty( $field['value'] ) ) {
                        $flag_empty_option = false;
                        echo $field['label'] . ': ' . $field['value'] . ' ';
                    }
                }
                if ( $flag_empty_option ) {
                    echo '<a href="' . esc_url( get_permalink() ) . '">' . __( 'Подробнее' ) . '</a> ';
                }
                echo '</p > ';
            }
        }
        ...
        if ( is_single() ) {
            ...
            wp_link_pages(
                array(
                    'before' => ' < div class="page-links" > ' . esc_html__( 'Страницы:', 'capri - lite' ),
                    'after'  => ' </div > ',
                )
            );
