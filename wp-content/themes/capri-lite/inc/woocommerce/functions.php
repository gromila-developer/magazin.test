 ...
 * @return mixed
 */
function capri_loop_columns() {
	if ( is_active_sidebar( 'sidebar-shop' ) ) {
		return apply_filters( 'capri_loop_columns', 3 ); // 3 products per row
	}

	return apply_filters( 'capri_loop_columns', 4 ); // 4 products per row
}
add_filter( 'loop_shop_columns', 'capri_loop_columns' );
